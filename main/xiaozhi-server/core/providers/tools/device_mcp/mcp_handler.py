...
class MCPClient:
    """The device-side MCP client is used to manage MCP status and tools."""
...
async def handle_mcp_message(conn, mcp_client: MCPClient, payload: dict):
    ...
    if not isinstance(payload, dict):
        logger.bind(tag=TAG).error("MCP message is missing payload field or has incorrect format")
        return
    ...
    if "result" in payload:
        ...
        if msg_id == 1:  # mcpInitializeID
            ...
            logger.bind(tag=TAG).debug("Initialization complete, starting to request MCP tools list")
            ...
            if isinstance(result, dict) and "tools" in result:
                ...
                for tool_data in mcp_client.tools.values():
                    if "description" in tool_data:
                        description = tool_data["description"]
                        # Iterate over all tool names for replacement
                        for (
                            sanitized_name,
                            original_name,
                ...
                if next_cursor:
                    ...
                    # Refresh tool cache to ensure MCP tools are included in the function list
...
async def send_mcp_initialize_message(conn):
    ...
    # Generate token from secret key
                ...
                try:
                    ...
                    try:
                        ...
                        if len(json_objects) > 1:
                            ...
                            raise ValueError(f"JSON parsing failed for arguments: {args}")
                    except Exception as e:
                        logger.bind(tag=TAG).error(
                            f"JSON parsing failed for arguments: {str(e)}, original arguments: {args}"
                        )
                        raise ValueError(f"JSON parsing failed for arguments: {str(e)}")
        elif isinstance(args, dict):
            arguments = args
        else:
            raise ValueError(f"Invalid argument type, expected string or dict, got: {type(args)}")
        # Ensure the arguments are of dictionary type
        if not isinstance(arguments, dict):
            raise ValueError(f"Arguments must be of dictionary type, actual type: {type(arguments)}")
        ...
        if isinstance(raw_result, dict):
            if raw_result.get("isError") is True:
                error_msg = raw_result.get(
                    "error", "Tool call returned an error, but no specific error message was provided"
                )
