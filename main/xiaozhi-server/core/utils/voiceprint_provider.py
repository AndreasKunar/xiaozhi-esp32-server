...
class VoiceprintProvider:
    ...
    def __init__(self, config: dict):
        ...
        # Voiceprint recognition similarity threshold, default is 0.4
        self.similarity_threshold = float(config.get("similarity_threshold", 0.4))
        
        # Parse API URL and key
        self.api_url = None
        self.api_key = None
        self.speaker_ids = []
    ...
    def _check_server_health(self) -> bool:
        ...
        # Cache the result using the global cache manager
    ...
    async def identify_speaker(self, audio_data: bytes, session_id: str) -> Optional[str]:
        ...
        if not self.enabled or not self.api_url or not self.api_key:
            logger.bind(tag=TAG).debug("Voiceprint recognition feature is disabled or not configured, skipping identification")
            return None
            ...
            async with aiohttp.ClientSession(timeout=timeout) as session:
                async with session.post(self.api_url, headers=headers, data=data) as response:
                    ...
                    if response.status == 200:
                        ...
                        logger.bind(tag=TAG).info(f"Voiceprint recognition elapsed time: {total_elapsed_time:.3f}s")
