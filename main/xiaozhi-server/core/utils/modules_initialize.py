...
def initialize_modules(
    ...
    if init_tts:
        ...
        logger.bind(tag=TAG).info(f"Initialized component: tts successfully {select_tts_module}")
    ...
    if init_llm:
        ...
        logger.bind(tag=TAG).info(f"Initialized component: llm successfully {select_llm_module}")
    ...
    if init_intent:
        ...
        logger.bind(tag=TAG).info(f"Initialized component: intent successfully {select_intent_module}")
    ...
    if init_memory:
        ...
        logger.bind(tag=TAG).info(f"Initialized component: memory successfully {select_memory_module}")
    ...
    if init_vad:
        ...
        logger.bind(tag=TAG).info(f"Initialized component: vad successfully {select_vad_module}")

    # Initialize ASR module
    if init_asr:
        select_asr_module = config["selected_module"]["ASR"]
        modules["asr"] = initialize_asr(config)
        logger.bind(tag=TAG).info(f"Initialized component: asr successfully {select_asr_module}")
    return modules
...
def initialize_asr(config):
    ...
    logger.bind(tag=TAG).info("Initialized component: asr successfully")
    return new_asr
...
def initialize_voiceprint(asr_instance, config):
    ...
    try:
        ...
        logger.bind(tag=TAG).info("ASR module voiceprint recognition feature has been dynamically enabled")
        logger.bind(tag=TAG).info(f"Configured number of speakers: {len(voiceprint_config['speakers'])}")
        return True
    except Exception as e:
        logger.bind(tag=TAG).error(f"Failed to dynamically initialize voiceprint recognition feature: {str(e)}")
        return False
