...
class ConnectionHandler:
    def __init__(
        ...
        self.last_bind_prompt_time = 0  # Timestamp of the last binding prompt played (seconds)
        self.bind_prompt_interval = 60  # Interval for playing binding prompts (seconds)
        ...
        # In the future, this can be modified to adjust ASR and TTS reporting; currently, both are enabled by default
        ...
        # Because in actual deployment, a public local ASR may be used, variables cannot be exposed to the public ASR
        # Therefore, variables related to ASR need to be defined here and belong to the private variables of the connection
        self.asr_audio = []
        self.asr_audio_queue = queue.Queue()
        self.current_speaker = None  # Store the current speaker
        self.current_language_tag = None  # Store the current ASR recognized language tag
        ...
        # Should the connection be closed after the chat ends?
        ...
        )  # Add 60 seconds on top of the original first closure for a second closure
    ...
    async def handle_connection(self, ws):
        try:
            ...
            # Read the sampling rate from the configuration.
            self.sample_rate = self.welcome_msg["audio_params"]["sample_rate"]
            self.logger.bind(tag=TAG).info(f"Configure the output audio sampling rate: {self.sample_rate}")

            # Initialize configuration and components in the background (non-blocking to the main loop)
    ...
    async def _save_and_close(self, ws):
        ...
        try:
            ...
            try:
                ...
                self.logger.bind(tag=TAG).error(
                    f"Error closing connection after saving memory: {close_error}"
                )
    ...
    async def _route_message(self, message):
        ...
        if not self.bind_completed_event.is_set():
            # The real status has not been obtained yet, wait until it is obtained or timeout
            try:
                await asyncio.wait_for(self.bind_completed_event.wait(), timeout=1)
            except asyncio.TimeoutError:
                # Timeout still did not get the real status, discard the message
                await self._discard_message_with_bind_prompt()
                return

        # The real status has been obtained, check if binding is needed
        if self.need_bind:
            # 需要绑定，丢弃消息
            await self._discard_message_with_bind_prompt()
            return

        # Binding is not needed, continue processing the message
        ...
        if isinstance(message, str):
            ...
            # If no header processing is needed or there is no header, process the raw message directly
    ...
    async def _process_mqtt_audio_message(self, message):
        ...
        try:
            ...
            if audio_length > 0 and len(message) >= 16 + audio_length:
                ...
                # No specified length or invalid length, process the remaining data after removing the header
        ...
        # If processing fails, returning False indicates that further processing is required.
        return False
    ...
    def _process_websocket_audio(self, audio_data, timestamp):
        ...
        if timestamp >= self.last_processed_timestamp:
            ...
            # Out-of-order packets, temporarily store
    ...
    async def handle_restart(self, message):
        ...
        try:
            ...
            def restart_server():
                ...
                time.sleep(1)
                self.logger.bind(tag=TAG).info("Restart the server...")
                subprocess.Popen(
            ...
            self.logger.bind(tag=TAG).error(f"Reboot failed: {str(e)}")
            await self.websocket.send(
                json.dumps(
                    {
                        "type": "server",
    ...
    def _initialize_components(self):
        try:
            ...
            # Open speech synthesis channels
            ...
            if self.config.get("prompt") is not None:
                ...
                # Initialize using quick prompts
                prompt = self.prompt_manager.get_quick_prompt(user_prompt)
                self.change_system_prompt(prompt)
                self.logger.bind(tag=TAG).info(
                    f"Quickly initialized components: prompt succeeded {prompt[:50]}..."
                )
            ...
            # Open speech recognition channels
            asyncio.run_coroutine_threadsafe(
    ...
    def _init_report_threads(self):
        ...
        if self.report_thread is None or not self.report_thread.is_alive():
            ...
            self.logger.bind(tag=TAG).info("TTS report thread has started")
        ...
        try:
            ...
            if voiceprint_config:
                ...
                if voiceprint_provider is not None and voiceprint_provider.enabled:
                    ...
                    self.logger.bind(tag=TAG).info("Voiceprint recognition has been dynamically enabled for the connection")
                else:
    ...
    async def _background_initialize(self):
        """Initialize configuration and components in the background (completely non-blocking main loop)"""
        try:
    ...
    async def _initialize_private_config_async(self):
        """Asynchronously get differentiated configuration from the API (async version, non-blocking main loop)"""
        ...
        try:
            ...
            self.need_bind = True
            self.logger.bind(tag=TAG).error(f"Asynchronous retrieval of differential configuration failed: {e}")
            private_config = {}
        ...
        # Use run_in_executor to execute initialize_modules in a thread pool to avoid blocking the main loop
        try:
    ...
    def _initialize_memory(self):
        ...
        # Use mem_local_short mode
            ...
            if memory_llm_name and memory_llm_name in self.config["LLM"]:
                ...
                self.logger.bind(tag=TAG).info("Using main LLM as the intent recognition model")
    ...
    def chat(self, query, depth=0):
        ...
        # For the top level, create a new session ID and send the FIRST request
        if depth == 0:
        ...
        MAX_DEPTH = 5
        force_final_answer = False  # Flag to force final answer

        if depth >= MAX_DEPTH:
            self.logger.bind(tag=TAG).debug(
                f"Reached maximum tool call depth {MAX_DEPTH}, will force final answer based on existing information"
            )
            ...
            self.dialogue.put(
                Message(
                    role="user",
                    content="[System Prompt] Reached the maximum tool call limit, please provide the final answer based on all the information obtained so far. Do not attempt to call any more tools.",
                )
            )

        # Define intent functions
        functions = None
        # Disable tool calls and force LLM to answer directly when maximum depth is reached
        if (
        ...
        for response in llm_responses:
            ...
            # Get emotion expression from LLM response, only once at the beginning of a conversation round
        ...
        if tool_call_flag:
            ...
            if not bHasError and len(tool_calls_list) > 0:
                # If the large model needs to process a round first, add related processed log information
        ...
        if depth == 0:
            ...
            self.llm_finish_task = True
            # Use lambda for delayed evaluation, only execute get_llm_dialogue() at DEBUG level
            self.logger.bind(tag=TAG).debug(
                lambda: json.dumps(
    ...
    def _handle_function_result(self, tool_results, depth):
        ...
        for result, tool_call_data in tool_results:
            if result.action in [
                ...
                Action.NOTFOUND,
                Action.ERROR,
            ]:  # Directly respond to the frontend
                text = result.response if result.response else result.result
                self.tts.tts_one_sentence(self, ContentType.TEXT, content_detail=text)
                self.dialogue.put(Message(role="assistant", content=text))
            elif result.action == Action.REQLLM:
                # Collect tools that require LLM processing
    ...
    def _report_worker(self):
        ...
        while not self.stop_event.is_set():
            try:
                # Get data from the queue, set timeout to periodically check stop event
    ...
    def _process_report(self, type, text, audio_data, report_time):
        ...
        try:
            # Execute asynchronous reporting (run in event loop)
    ...
    async def close(self, ws=None):
        ...
        try:
            ...
            # Clean up function handler resources
            ...
            try:
                if ws:
                    # Securely check the WebSocket state and close it.
                    try:
    ...
    def clear_queues(self):
        ...
        if self.tts:
            self.logger.bind(tag=TAG).debug(
                f"Starting cleanup: TTS text queue size={self.tts.tts_text_queue.qsize()}, audio queue size={self.tts.tts_audio_queue.qsize()}"
            )
            ...
            # Reset the audio rate controller (cancel background tasks and clear queues)
            ...
            self.logger.bind(tag=TAG).debug(
                f"Cleanup finished: TTS text queue size={self.tts.tts_text_queue.qsize()}, audio queue size={self.tts.tts_audio_queue.qsize()}"
            )
                ...
                if last_activity_time > 0.0:
                    ...
                    if current_time - last_activity_time > self.timeout_seconds * 1000:
                        if not self.stop_event.is_set():
                            ...
                            # Ensure the stop event is set to prevent duplicate handling
                            self.stop_event.set()
                            # Use try-except to wrap the close operation to ensure it does not block due to exceptions
                            try:
        ...
        for tool_call in tools_call:
            ...
            if tool_index is None:
                if tool_call.function.name:
                    # If there is a function_name, it indicates a new tool call
