...
# Pre-buffered packet count, sent directly to reduce latency.
PRE_BUFFER_COUNT = 5
...
async def sendAudioMessage(conn, sentenceType, audios, text):
    ...
    # Send stop message (if it is the last text)
...
async def _wait_for_audio_completion(conn):
    """
    Wait for the audio queue to be cleared and for the pre-buffered packet to finish playing.
    ...
    if hasattr(conn, "audio_rate_controller") and conn.audio_rate_controller:
        ...
        conn.logger.bind(tag=TAG).debug(
            f"Waiting for audio sending to complete, {len(rate_controller.queue)} packets still in the queue"
        )
        await rate_controller.queue_empty_event.wait()

        # Wait for pre-buffered packets to finish playing
        # The first N packets are sent directly, plus 2 network jitter packets, so we need to wait extra for them to finish playing on the client side
...
async def sendAudio(conn, audios, frame_duration=AUDIO_FRAME_DURATION):
    ...
    Args:
        ...
        frame_duration: Frame duration (milliseconds), default is the global constant AUDIO_FRAME_DURATION
    """
...
def _get_or_create_rate_controller(conn, frame_duration, is_single_packet):
    ...
    Args:
        ...
        is_single_packet: Whether it is single packet mode (True: TTS streaming single packet, False: batch packets)
    ...
    if not hasattr(conn, "audio_rate_controller"):
        ...
        # Background sending task has stopped, so it needs to be reset
        if (
...
async def _send_audio_with_rate_control(
    ...
    for packet in audio_list:
        ...
        if flow_control["packet_count"] < PRE_BUFFER_COUNT:
            ...
            # Dynamic flow control mode: only add to queue, background loop is responsible for sending
...
async def send_stt_message(conn, text):
    ...
    # Parse JSON format to extract actual user speech content
    display_text = text
    try:
        ...
        if text.strip().startswith("{") and text.strip().endswith("}"):
            ...
            if isinstance(parsed_data, dict) and "content" in parsed_data:
                # If it is a JSON format containing speaker information, only display the content part
