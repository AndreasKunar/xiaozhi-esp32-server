...
class SuppressInvalidHandshakeFilter(logging.Filter):
    """Filter out invalid handshake error logs (such as HTTPS access to WS ports)."""
...
class WebSocketServer:
    ...
    async def _handle_connection(self, websocket):
        ...
        if headers.get("device-id", None) is None:
            ...
            if "device-id" not in query_params:
                await websocket.send("Port is normal. If you want to test the connection, please use test_page.html")
                await websocket.close()
                return
            else:
        ...
        """Handle new connection, create a separate ConnectionHandler each time"""
        ...
        handler = ConnectionHandler(
            ...
            self._memory,
            self._intent,
            self,  # Pass the current server instance
        )
        try:
            ...
            try:
                ...
                self.logger.bind(tag=TAG).error(
                    f"Error occurred while forcibly closing the connection on the server side: {close_error}"
                )
    ...
    async def _http_response(self, websocket, request_headers):
        ...
        if request_headers.headers.get("connection", "").lower() == "upgrade":
            ...
            # If it is a regular HTTP request, return "server is running"
    ...
    async def _handle_auth(self, websocket):
        ...
        if self.auth_enable:
            ...
            if self.allowed_devices and device_id in self.allowed_devices:
                # If the device is in the whitelist, do not verify the token and allow access directly
                return
            else:
