...
class OTAHandler(BaseHandler):
    ...
    def _get_websocket_url(self, local_ip: str, port: int) -> str:
        ...
        if "your" not in websocket_config and "你的" not in websocket_config:  # is not default placeholder
    ...
    async def handle_post(self, request):
        ...
        try:
            ...
            if mqtt_gateway_endpoint:  # If a non-empty string is configured
                # Try to get device model from request data (already parsed above)
                try:
                ...
                if signature_key:
                    ...
                    if not password:
                        password = ""  # If signature fails, leave empty; device decides whether to allow no password
                else:
                    self.logger.bind(tag=TAG).warning("Missing MQTT signature key, password left empty")

                # Construct MQTT configuration (directly use mqtt_gateway string)
                return_json["mqtt"] = {
                ...
                self.logger.bind(tag=TAG).info(f"MQTT gateway configuration issued for device {device_id}")

            else:  # MQTT gateway not configured, issue WebSocket
                ...
                self.logger.bind(tag=TAG).info(
                    f"MQTT gateway not configured, issued WebSocket configuration for device {device_id}"
                )
            ...
            try:
                ...
                self.logger.bind(tag=TAG).info(
                    f"Found {len(candidates)} firmware candidates for model {device_model}"
                )
                ...
                if chosen_url:
                    ...
                    self.logger.bind(tag=TAG).info(
                        f"Firmware {chosen_version} issued to device {device_id} [If the URL prefix is incorrect, please check the server.vision_explain in the configuration file] -> {chosen_url} "
                    )
                else:
                    self.logger.bind(tag=TAG).info(
    ...
    async def handle_get(self, request):
        ...
        try:
            ...
            message = f"OTA interface is running normally, the websocket URL sent to the device is: {websocket_url}"
