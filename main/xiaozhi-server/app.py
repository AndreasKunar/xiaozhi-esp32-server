...
async def wait_for_exit() -> None:
    """
    Blocks until a Ctrl-C / SIGTERM is received.
    ...
    if sys.platform != "win32":  # Unix / macOS
        ...
        # let KeyboardInterrupt bubble up to asyncio.run to avoid process exit blocking caused by legacy normal threads
        try:
...
async def main():
    ...
    # Priority of auth_key: config file server.auth_key > manager-api.secret > auto-generated
    ...
    if not auth_key or len(auth_key) == 0 or "you" in auth_key or "你" in auth_key:  # check for default placeholder
    ...
    # Start global GC manager (cleans every 5 minutes)
    ...
    logger.bind(tag=TAG).info(
        "Vision analysis interface:\thttp://{}:{}/mcp/vision/explain",
        get_local_ip(),
        port,
    )
    mcp_endpoint = config.get("mcp_endpoint", None)
    if mcp_endpoint is not None and ("你" not in mcp_endpoint or "you" not in mcp_endpoint):    # is not default placeholder
        ...
        if validate_mcp_endpoint(mcp_endpoint):
            ...
            config["mcp_endpoint"] = "your MCP websocket address"
    ...
    try:
        ...
        # Wait for the task to terminate (timeout must be added)
        await asyncio.wait(
