...
export function checkOpusLoaded() {
    try {
        // Check if the Module exists (global variables exported from the local library).
        if (typeof Module === 'undefined') {
            throw new Error('Opus library not loaded, Module object does not exist');
        }

        // Try using Module.instance first (export method at the last line of libopus.js)
        if (typeof Module.instance !== 'undefined' && typeof Module.instance._opus_decoder_get_size === 'function') {
            // Use Module.instance object to replace the global Module object
        ...
        // If there is no Module.instance, check the global Module functions
        ...
        throw new Error('Opus decoding functions not found, Module structure may be incorrect');
    } catch (err) {
        log(`Opus library failed to load, please check if libopus.js file exists and is correct: ${err.message}`, 'error');
    }
}
        ...
        opusEncoder = {
            ...
            init: function () {
                try {
                    ...
                    // Enable DTX (Discontinuous Transmission, do not transmit silence frames)
