...
export class StreamingContext {
    constructor(opusDecoder, audioContext, sampleRate, channels, minAudioDuration) {
        ...
        this.playing = false;     // Whether it is playing
        this.endOfStream = false; // Whether the end signal has been received
        ...
        // Initialize analyser node (for Live2D use)
    ...
    // Get pending buffer queue to process, single-threaded: no safety issues when audioBufferQueue is continuously updated
    ...
    convertInt16ToFloat32(int16Data) {
        ...
        for (let i = 0; i < int16Data.length; i++) {
            // Convert range [-32768,32767] to [-1,1], consistently using 32768.0 to avoid asymmetric distortion
    ...
    // Get pending play count (converted to packet count, 960 samples per packet)
    getPendingPlayCount() {
        // Calculate samples already in the queue
        const queuedSamples = this.activeQueue.length + this.queue.length;

        // Calculate samples that have been scheduled but not yet played (in the Web Audio buffer)
    ...
    clearAllBuffers() {
        ...
        // Clear all queues (use clear method to maintain object references)
    ...
    // Get analyser node (for Live2D use)
    getAnalyser() {
    ...
    // Start playing audio
    async startPlaying() {
        this.scheduledEndTime = this.audioContext.currentTime; // Track the end time of scheduled audio
        ...
        while (true) {
            ...
            // Continuously play audio from the queue, one chunk at a time
            while (this.playing && this.queue.length > 0) {
                // Play 120ms of audio each time (2 Opus frames)
                ...
                // Precise scheduling of playback time
                const currentTime = this.audioContext.currentTime;
                ...
                // Update the scheduled end time for the next audio chunk
                const duration = audioBuffer.duration;
                this.scheduledEndTime = startTime + duration;
                this.lastPlayTime = startTime;

                // If the queue has insufficient data, wait for new data
