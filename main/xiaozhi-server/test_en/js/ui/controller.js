...
class UIController {
        ...
        if (recordBtn) {
            ...
            if (isConnected && microphoneAvailable) {
                ...
                if (!microphoneAvailable) {
                    recordBtn.title = window.isHttpNonLocalhost ? 'Currently, recording is not available due to HTTP access; only text interaction is possible' : 'Microphone not available';
                } else {
    ...
    updateMicrophoneAvailability(isAvailable, isHttpNonLocalhost) {
        ...
        if (!isAvailable) {
            ...
            recordBtn.querySelector('.btn-text').textContent = 'Record';
            recordBtn.title = isHttpNonLocalhost ? 'Currently, recording is not available due to HTTP access; only text interaction is possible' : 'Microphone not available';
    ...
    startAIChatSession() {
        this.addChatMessage('Connection successful, start chatting~😊', false);
        // Check microphone availability and show error messages if needed
        if (!window.microphoneAvailable) {
            if (window.isHttpNonLocalhost) {
                this.addChatMessage('⚠️ Due to the current HTTP access, audio recording is not possible; only text interaction is supported', false);
            } else {
                this.addChatMessage('⚠️ Microphone not available, please check permission settings; only text interaction is supported', false);
            }
        }
    ...
    addMCPTool() {
        ...
        toolDiv.innerHTML = `
            <div class="property-item">
                ...
                <button class="remove-property" onclick="uiController.removeMCPTool('${toolId}')">Remove</button>
            </div>
        `;
