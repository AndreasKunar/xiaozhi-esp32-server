export default class BlockingQueue {
    ...
    enqueue(item, ...restItems) {
        ...
        // If there are additional arguments, batch process all items
        else {
    ...
    async dequeue(min = 1, timeout = Infinity, onTimeout = null) {
        // 1. If empty, wait for the first item to arrive (all calls share the same promise)
    ...
    /* Clear the queue (maintain object reference, do not affect waiters) */
    clear() {
        this.#items.length = 0;
    }
}